      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <motion.h1 
            ...
            className="text-4xl md:text-5xl font-black text-navy tracking-tight mb-4"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {organizations.map((org, index) => (
            <motion.div
              ...
              className="bg-white/60 backdrop-blur-xl border border-white/60 rounded-3xl p-8 shadow-sm flex flex-col h-full relative overflow-hidden"
            >
              {/* Decorative top accent */}
              <div className={`absolute top-0 left-0 w-full h-2 bg-gradient-to-r ${org.color}`}></div>
              
              <div className="flex-grow">
                <div className="relative w-full h-32 mb-6 flex items-center justify-center bg-white rounded-xl p-3 overflow-hidden shadow-sm border border-slate-100">
                  <img src={org.logo} alt={`${org.name} Logo`} className={`max-h-full max-w-full object-contain transition-transform ${org.imageClass || ''}`} />
                </div>
                
                <h2 className="text-2xl font-bold text-navy mb-1">{org.name}</h2>
                <h3 className="text-sm font-semibold text-slate-500 mb-4">{org.fullName}</h3>
              ...
              <div className="mt-auto space-y-3 pt-6 border-t border-slate-200/50">
                <a 
                  href={`mailto:${org.email}`} 
                  className="flex items-center text-slate-600 hover:text-navy font-semibold transition-colors group"
                >
                  <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center mr-3 group-hover:bg-slate-100 transition-colors shadow-sm">
                ...
                <a 
                  ...
                  className="flex items-center text-slate-600 hover:text-pink-600 font-semibold transition-colors group"
                >
                  <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center mr-3 group-hover:bg-pink-50 transition-colors shadow-sm">
